...
In this tutorial we'll create super simple web browser
for web developers using Python PyQt framework.
PyQt is a set of Python bindings for Qt framework.
What is Qt then? Qt (pronounced *cute*) is C++ framework used to create
...
Most important thing for this post is that you can just create Webkit browser 
with PyQt. This is possible because Qt comes with a port Webkit included,
you just need to import classes you need from PyQt.
...
Let's start with simplest possible use case of PyQt Webkit, let's say you
just want to load some url, open a window and render page in this window. 
...
is already better than Lynx because it renders JavaScript. But it's not much better
because you can only pass urls from command line when you invoke it. We 
...
into text box, and browser will load this url. We will use QLineEdit widget for 
...
At this point you have bare-bones browser. You can enter url into input box and 
...
Our Python browser should have some developer tools too. Let's add them.

We'll start by adding something similar to Chrome "network" tab in dev tools.
...
Table will have header with field names, it will also auto-resize each time
...
QVariant not int and when you convert to int it returns tuple. Sounds like bit of 
work for very straightforward task after all it is just geting HTTP code of response.
